...
| `parut_mvp_v9.postman_collection.json` | MVP 시연 순서와 자동 변수 저장 스크립트 | O — 최종 통합 시나리오 검증에 사용 |
| `local_mvp_v9.postman_environment.json` | MVP 시연용 환경 변수 서식 | O — **값은 비운 상태로만** |
...
MVP 시연은 `parut_mvp_v9.postman_collection.json`과 `local_mvp_v9.postman_environment.json`을 임포트하고 `PARUT MVP LOCAL V9` 환경을 선택한다. 요청은 번호 순서대로 실행하며 선택 시연 폴더는 필요한 경우에만 사용한다. V9는 회원가입과 판매자 승인, 일반 상품 주문과 결제, 배송과 구매확정, 타임딜 주문과 결제 흐름까지 통합 검증했다.
